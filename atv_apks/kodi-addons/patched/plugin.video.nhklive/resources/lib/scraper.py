...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
      ...
      try:
         response = requests.get(
            'https://nwapi.nhk.jp/nhkworld/vodpglist/v8b/en/voice/list.json',
            headers=self.defaultHeaders,
            timeout=15)
         response.raise_for_status()
         b = response.json()['vod_programs']['programs']
      except (requests.RequestException, ValueError, KeyError) as exc:
         # NHK retired this VOD API, but the independent live HLS stream still works.
         xbmc.log('NHK Live: VOD catalogue unavailable: %s' % exc, xbmc.LOGWARNING)
         return ilist
  ...
  def getAddonShows(self,url,ilist):
      ...
      try:
         ...
         xbmc.log('NHK Live: EPG unavailable, using direct live stream: %s' % exc, xbmc.LOGWARNING)
         infoList = {
            'mediatype': 'episode',
         ...
         ilist = self.addMenuItem(
            'NHK WORLD-JAPAN Live', 'GV', ilist, nhkurl,
            self.addonIcon, self.addonFanart, infoList, isFolder=False)
         return ilist
